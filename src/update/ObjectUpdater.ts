...
type GetRecordKeyType<Type> = Type extends Record<infer Key, unknown>
    ? Key
    : never;

type GetRecordValueType<Type> = Type extends Record<string | number | symbol, infer Return>
    ? Return
    : never;
...
type ObjectUpdater<T, TParent> = {
...
} &
    {
        ...
        [K in keyof T as T[K] extends Record<string | number | symbol, unknown> ? `${Uncapitalize<string & K>}AsRecord` : never]:
        () => RecordUpdater<T[K], ObjectUpdater<T, TParent>>
    } &
    {
        //special case for objects
        [K in keyof T as T[K] extends Record<string | number | symbol, unknown> ? `${Uncapitalize<string & K>}AsObject` : never]:
        () => ObjectUpdater<T[K], ObjectUpdater<T, TParent>>
    } &
{ end: () => TParent }
